...
function IncBreakdown() {
  ...
  const fetchTransactions = async () => {
    try {
      // Get all income transactions without date filter
      const q = query(
      ...
      >
        ...
        <div className="px-6 mt-2">
          <div className="flex justify-between items-center py-2 border-b border-slate-700/50">
            <span className="text-sm text-slate-400">Total Incomes:</span>
        ...
        {incomeData.length > 0 ? (
          ...
          <div className="flex justify-center items-center h-72 text-slate-400">
            No incomes recorded for {currentMonthName}
          </div>
        )}
      </motion.div>
    </div>
